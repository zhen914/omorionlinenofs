...
(function () {
    ...
    Window_SlotMachine.prototype.refreshCoin = function() {
		...
		this.coinContents.fontSize = 22;
        this.drawCoinText(this._coin, 0, 2, this.contents.measureTextWidth("9999"), "right");
    };
    
    Window_SlotMachine.prototype.refreshBet = function() {
        this.betContents.clear();
		this.betContents.fontSize = 22;
        this.drawBetText(this._bet, 0, 2, this.contents.measureTextWidth("999"), "right");
    };
	...
	Window_SlotsIntroduction.prototype.setText = function(text) {
		...
		if (text && text.contains("<br>")) {
			...
			var textState = { index: 0, x: 8, y: y, left: 8 };
			textState.text = this.convertEscapeCharacters(line);
			var height = this.calcTextHeight(textState, false);
				this.drawTextEx(line, 8, y + (height / 2), this.contents.width);
				y += height;
			}
		} else {
			var textState = { index: 0, x: 8, y: -6, left: 8 };
			textState.text = this.convertEscapeCharacters(text);
			var height = this.calcTextHeight(textState, false);
			this.drawTextEx(text, 8, -6 + (height / 2), this.contents.width);
		}
	};
