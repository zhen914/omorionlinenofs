...
(function($) {
	...
	isTextInCurrentLanguage = (text) => {
		...
		switch(lang) {
			...
			case "jp":
				...
				break
			case "kr":
				let koreanOnly = /(^[\u3131-\u314e|\u314f-\u3163|\uac00-\ud7a3|0-9!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?☆ ]*$)/
	...
	Graphics = class extends Graphics {
		...
		static processErrorStackMessage(stack) {
			...
			data.unshift(' <a href="#" onclick="Graphics.openSteamBugPage()" style="color:green;">BOCHOK POTIK NAHUI</a><br>');
			for (var i = 1; i < data.length; ++i) {
				data[i] = `<span style='color: red'>${data[i].replace(/[\(](.*[\/])/, '(')}</span>`;
			}
			return data;
		}
	}
		...
		if (myLanguageIndex != -1)
		{
			...
			for (let entryNum = 0; entryNum < namesPerLanguage.length; entryNum++)
			{
				for (let languageID = 0; languageID < namesPerLanguage[entryNum].length; languageID++)
				{
					...
					if (name == namesPerLanguage[entryNum][languageID])
					{ // Name from other language detected
	...
	_old_loadGameWithoutRescue = DataManager.loadGameWithoutRescue;
	DataManager.loadGameWithoutRescue = function (savefileId)
	{
		let result = _old_loadGameWithoutRescue.call(this, savefileId);
	...
	DataManager.loadSavefileInfo = function (savefileId)
	{
		...
		if (globalInfo && globalInfo[savefileId]) {
			...
			console.log(globalInfo[savefileId].actorData.name)
			console.log(!isTextInCurrentLanguage(globalInfo[savefileId].actorData.name))
	...
	// Message Debugger global variables
	var MessageDebugger = MessageDebugger || {};
	MessageDebugger.yamlFile = "";
	MessageDebugger.yamlFileIndex = 0;
	MessageDebugger.yamlStringIndex = new Map();
	MessageDebugger.searchFile = "";
	MessageDebugger.searchString = "";
	MessageDebugger.battleText = false;

	// Check for debug keys
	MessageDebugger.Scene_Base_update = Scene_Base.prototype.update;
	Scene_Base.prototype.update = function ()
	{
		MessageDebugger.Scene_Base_update.call(this);
		if ($gameTemp.isPlaytest() && $gameSystem)
		{
			Input.keyMapper[118] = 'textdebug'; // 118: F7
			Input.keyMapper[121] = 'reloadlang'; // 121: F10

			if (Input.isTriggered('reloadlang'))
			{
				// Reload relevant data JSONs and language files
				SoundManager.playShop();
				DataManager.loadDataFile('$dataSkills', 'Skills.json');
				...
				LanguageManager.loadLanguageFiles(LanguageManager.defaultLanguage());
			};

			if (Input.isTriggered('textdebug') && !!SceneManager._scene._mapLoaded && !$gameMap.isEventRunning() && !MessageDebugger.open)
			{
				// Open message debug menu
				SoundManager.playShop();
				MessageDebugger.open = true;
				MessageDebugger.yamlFile = "";
				SceneManager.push(Scene_MessageDebugger);
			};
		};
	};
	...
	function Scene_MessageDebugger()
	{
		this.initialize.apply(this, arguments);
	}

	Scene_MessageDebugger.prototype = Object.create(Scene_MenuBase.prototype);
	Scene_MessageDebugger.prototype.constructor = Scene_MenuBase;

	Scene_MessageDebugger.prototype.initialize = function ()
	{
		Scene_MenuBase.prototype.initialize.call(this);
		this._choiceWindow = new MessageDebugger_Window_ChoiceList();
		this.addChild(this._choiceWindow);
	}

	Scene_MessageDebugger.prototype.create = function ()
	{
	...
	function MessageDebugger_Window_ChoiceList()
	{
		this.initialize.apply(this, arguments);
	}

	MessageDebugger_Window_ChoiceList.prototype = Object.create(Window_Selectable.prototype);
	MessageDebugger_Window_ChoiceList.prototype.constructor = MessageDebugger_Window_ChoiceList;

	// Initialization
	MessageDebugger_Window_ChoiceList.prototype.initialize = function ()
	{
		this._list = [];
		...
		this.select(MessageDebugger.yamlFile === "" ? MessageDebugger.yamlFileIndex : MessageDebugger.yamlStringIndex.get(MessageDebugger.yamlFile)); // Select memorized index
		this.refresh();
	};

	// Set up window position and size
	MessageDebugger_Window_ChoiceList.prototype.updatePlacement = function ()
	{
		this.x = 0;
		this.y = 0;
	...
	// Generate choice list for either all YAML files or strings within the selected YAML file
	MessageDebugger_Window_ChoiceList.prototype.makeCommandList = function ()
	{
		this._list = [];
		if (MessageDebugger.yamlFile === "")
			this.makeFileList();
		else
			this.makeMessageList(MessageDebugger.yamlFile);
	};

	// Make list out of all YAML files in current language's directory
	MessageDebugger_Window_ChoiceList.prototype.makeFileList = function ()
	{
		...
		for (var i = 0; i < dirList.length; i++)
		{
			...
			if (format === '.yaml')
			{
				Window_Command.prototype.addCommand.call(this, filename, filename);
				continue;
			};
		};
	};

	// Make list out of all strings in selected YAML file
	MessageDebugger_Window_ChoiceList.prototype.makeMessageList = function (file)
	{
		LanguageManager.reloadLanguageFile(file);
		var fileData = LanguageManager._data[LanguageManager.defaultLanguage()].text[file];
		if (fileData === null || fileData === undefined) return;
		var keys = Object.keys(fileData);
		for (var i = 0; i < keys.length; i++)
		{
			// Set preview text based on string contents, and remove things that mess up display
			var previewText = "";
			if (fileData[keys[i]].text)
			{
				previewText = ": " + fileData[keys[i]].text;
				var extraCodes = ['SINV', 'SINH', 'SHAKE', 'QUAKE', 'RAINBOW', 'LETAG', 'LESTP', 'LESYNCH', 'LETMR', 'LECLEAR', 'Com'];
				for (var k = 0; k < extraCodes.length; k++)
				{ // Remove text effect and other codes
					previewText = previewText.replace(new RegExp('\\\\' + extraCodes[k] + '\\[[0-9]+\\]', 'gi'), "");
				};
				previewText = previewText.replace(/\\}/g, ""); // Remove \} (shrink text) codes to avoid text being parsed as "{...}" and showing as undefined
				previewText = this.convertEscapeCharacters(previewText); // Process remaining escape codes
			};

			Window_Command.prototype.addCommand.call(this, keys[i] + previewText, keys[i]);
		};
	};

	// Reload a specific language file
	LanguageManager.reloadLanguageFile = function (filename)
	{
		var language = LanguageManager._language;
		var path = require('path');
		var fs = require('fs');
	...
	// Return size of list
	MessageDebugger_Window_ChoiceList.prototype.maxItems = function ()
	{
		return this._list.length;
	};

	// Draw choice entry
	MessageDebugger_Window_ChoiceList.prototype.drawItem = function (index)
	{
	...
	// Process input
	MessageDebugger_Window_ChoiceList.prototype.update = function ()
	{
		Window_Selectable.prototype.processCursorMove.call(this);
		// Allow holding down page buttons to scroll
		...
		if (Input.isTriggered('textdebug'))
		{
			var stringMode = (MessageDebugger.yamlFile !== "");
			var searchKey = window.prompt("찾으려는 " + (!stringMode ? "파일" : "문자열 ID나 내용") + "을 입력해주세요.", !stringMode ? MessageDebugger.searchFile : MessageDebugger.searchString);
			if (searchKey == null || searchKey === "") return;
			if (!stringMode)
			{
				MessageDebugger.searchFile = searchKey;
			} else
			{
				MessageDebugger.searchString = searchKey;
			};
		...
		if (Input.isTriggered('cancel'))
		{
			if (MessageDebugger.yamlFile !== "")
			{ // String list
				// Back out of string list and recreate choice list
				MessageDebugger.yamlStringIndex.set(MessageDebugger.yamlFile, this.index()); // Memorize position in this file's string list
				MessageDebugger.yamlFile = "";
				this.makeCommandList();
				this.select(MessageDebugger.yamlFileIndex);
				this.refresh();
			} else
			{ // File list
				// Close the menu
				MessageDebugger.yamlFileIndex = this.index(); // Memorize position in file list
				this.close();
				MessageDebugger.open = false;
			};
		};
	};

	// OK button handler
	MessageDebugger_Window_ChoiceList.prototype.onChoice = function (index)
	{
		var choice = this._list[index].symbol;
		if (MessageDebugger.yamlFile === "")
		{ // File list
			// Enter string list for the selected file and recreate choice list
			MessageDebugger.yamlFileIndex = index; // Memorize position in file list
			if (!MessageDebugger.yamlStringIndex.has(choice)) MessageDebugger.yamlStringIndex.set(choice, 0); // Initialize position in this file's string list
			MessageDebugger.yamlFile = choice;
			this.makeCommandList();
			this.select(MessageDebugger.yamlStringIndex.get(choice));
			this.refresh();
		} else
		{ // String list
			// Temporarily close menu (don't set MessageDebugger.open) and show the selected message
			MessageDebugger.yamlStringIndex.set(MessageDebugger.yamlFile, index); // Memorize position in this file's string list
			this.close();
			LanguageManager.reloadLanguageFile(MessageDebugger.yamlFile);
			MessageDebugger.battleText = (MessageDebugger.yamlFile === "xx_battle_text"); // Set battle mode for battle strings
			$gameMessage.showLanguageMessage(MessageDebugger.yamlFile + "." + choice);
		};
	};

	// Search for file/string ID/text content, prioritizing exact matches
	MessageDebugger_Window_ChoiceList.prototype.doSearch = function (searchKey)
	{
		var stringMode = (MessageDebugger.yamlFile !== "");
		if (!stringMode && searchKey.includes("."))
		{ // Possible "filename.stringID" search term, search for filename
			searchKey = searchKey.split('.')[0];
		} else if (stringMode && searchKey.includes("."))
		{ // Possible "filename.stringID" search term
			var filename = searchKey.split('.')[0];
			if (filename === MessageDebugger.yamlFile)
			{ // Filename matches with current file, so search only for string ID
				searchKey = searchKey.replace(filename + ".", "");
			};
		};
		...
		do
		{
			...
			var name = this._list[index].name ? this._list[index].name : "";
			var symbol = this._list[index].symbol ? this._list[index].symbol : "";
			if (symbol === searchKey || name === searchKey)
			{
				this.select(index);
				return;
			} else if (firstMatch == -1 && (symbol.includes(searchKey) || name.includes(searchKey)))
			{
				firstMatch = index;
			}
	...
	// Close the menu
	MessageDebugger_Window_ChoiceList.prototype.close = function ()
	{
		SceneManager.pop();
	};

	// Override to set smaller textbox width in battle mode
	_old_Window_Message_windowWidth = Window_Message.prototype.windowWidth;
	Window_Message.prototype.windowWidth = function ()
	{
		if (MessageDebugger.battleText) { return 360 }
		return _old_Window_Message_windowWidth.call(this);
	};

	// After closing a message initiated by Message Debugger, reopen the menu
	_old_Window_Message_terminateMessage = Window_Message.prototype.terminateMessage;
	Window_Message.prototype.terminateMessage = function ()
	{
		_old_Window_Message_terminateMessage.call(this);
		if (MessageDebugger.open)
		{
			SceneManager.push(Scene_MessageDebugger);
		};
	};
			...
			if (action.isValid()) {
				...
				if(DataManager.isSkill(action.item())) {
					if(subject.mp < action.item().mpCost) {
						var lastChar = subject.name().toUpperCase()[subject.name().length - 1];
						var spaceRequired = lastChar >= 'A' && lastChar <= 'Z';
						this._logWindow.push("addText", subject.name().toUpperCase() + (spaceRequired ? ' ' : '') + "没有足够的活力了！");
	...
	$.parseNoEffectEmotion = function(tname, em) {
		if(em.toLowerCase().contains("afraid")) {
		  return target.name() + targetSp + "无法变得更害怕了！\r\n";
		}
		let finalString = `${tname}无法变得${em}`;
	...
	DataManager = class extends DataManager {
		...
		static loadDatabase() {
			...
			if(window['$externalNotesData'] === undefined) {
				...
				window['$externalNotesData'] = jsyaml.load(noteBuffer.toString());
			}

			if(window["$dataQuests"] === undefined) {
				let questBuffer = fs.readFileSync(base + '/data/Quests.yaml');
				window["$dataQuests"] = jsyaml.load(questBuffer.toString());
			}
		...
		static loadMapData(mapId) {
			...
			if(mapId > 0) {
				...
				fs.readFile(base + "/data/" + filename, (err, buffer) => {
					...
					window["$dataMap"] = JSON.parse(buffer.toString());
		...
		static loadLanguageFiles(language) {
			...
			for(let directory of dirList) {
				...
				if (format === ".yaml") {
					...
					this._data[language].text[filename] = jsyaml.load(buff.toString());
				}
			}
...
(function($) {
	...
	Window_Base.prototype.drawText = function (text, x, y, maxWidth, align) {
		this.contents.drawText(text, x, y + 2, maxWidth, this.lineHeight(), align);
	};
	//// 2022.2.17 ADD e ////
	...
	if(!!$._enable_2byte_linebreak) {
		_TDS_.TextEffects.Window_Base_processNormalCharacter = function(textState) {
			...
			if (!this._letterEffectsActive) textState.index -= 1; // 
	...
	Window_OmoMenuHelp.prototype.wordwrapWidth = function() {
		...
		if(!!this._item) {
			if(!!this._item.meta.IconIndex) {
				diff = 8 + (106 * this._iconRate) // 0
			}
		}
		return this.contents.width - diff
	}

	_old_Window_Message_newPage = Window_Message.prototype.newPage;
	Window_Message.prototype.newPage = function(textState) {
	  _old_Window_Message_newPage.call(this, textState);
	  if(!this.isEasternLocale()) {return;}
	  this.clearLetterEffectSprites();
	}

	//###############################################################################
	//
	// FIXING OMO MENU ITEM WINDOW SIZES
	//
	//###############################################################################
/*
	const _old_Scene_OmoMenuItem_createHelpWindow = Scene_OmoMenuItem.prototype.createHelpWindow
	Scene_OmoMenuItem.prototype.createHelpWindow = function() {
		_old_Scene_OmoMenuItem_createHelpWindow.call(this);
		if(!this._helpWindow.isEasternLocale()) {return;}
		this._helpWindow._showHeight += Math.floor(this._helpWindow.lineHeight() / 1.5)
	}

	const _old_Window_OmoMenuItemList_windowHeight = Window_OmoMenuItemList.prototype.windowHeight
	Window_OmoMenuItemList.prototype.windowHeight = function() {
		const val = !!this.isEasternLocale() ? Math.floor(this.lineHeight() / 1.5) : 0
		return _old_Window_OmoMenuItemList_windowHeight.call(this) + val;
	}
*/
})(Gamefall.JPWordWrap)
